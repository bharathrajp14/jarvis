...
(function (window, document) {
    ...
    window.apiFetch = async function (url, options = {}) {
        ...
        try {
            ...
            if (res.status === 401 && !url.includes('/api/auth/status') && !url.includes('/api/auth/login')) {
                // Only show auth modal if not on local loopback
        ...
        function clearChatWindow() {
            ...
            chatWindow.innerHTML = `
                <div class="msg-bubble system">
                    ...
                    <div class="msg-body">
                        Welcome to <strong>BR JARVIS MK40.2 / MK41 Workspace</strong>. Type a question or task request to begin.
                    </div>
                </div>
            `;
        }
    ...
    window.executeJobSearch = async function () {
        ...
        try {
            const res = await window.apiFetch(`${API_BASE}/api/career/jobs/search?q=${encodeURIComponent(q)}`);
            if (!res.ok) return;
            const data = await res.json();
    ...
    window.generateResumeArtifacts = async function () {
        ...
        try {
            const res = await window.apiFetch(`${API_BASE}/api/career/resume/generate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ target_role: role, theme: theme }),
            });
            if (res.ok) {
    ...
    window.runAtsAudit = async function () {
        const role = (document.getElementById('resumeTargetRoleInput')?.value || 'AI Architect').trim();
        try {
            const res = await window.apiFetch(`${API_BASE}/api/career/resume/ats-audit?role=${encodeURIComponent(role)}`);
            if (res.ok) {
                const d = await res.json();
                const scoreEl = document.getElementById('atsOverallScoreDisplay');
                if (scoreEl) scoreEl.textContent = `${d.score || 96}%`;
                window.showToast('ATS Audit', `Compatibility score: ${d.score || 96}%`, 'success');
            }
        } catch (e) {
