...
def get_or_create_master_key() -> bytes:
    ...
    try:
        ...
        print(f"[ContactSecurity] ⚠️ Failed to save keyfile: {e}")
    return key
        ...
        with _lock:
            if self.storage_path.exists():
                try:
                    ...
                    print(f"[ContactStore] ⚠️ Encrypted load error ({e}). Checking legacy file.")
            ...
            if self.legacy_path.exists():
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        self._contacts = {c.get("id", c.get("name", f"c_{i}")): c for i, c in enumerate(data)}
                    print(f"[ContactStore] 🔒 Migrating {len(self._contacts)} contacts to encrypted storage...")
                    self.save()
                    return
                except Exception as e:
                    print(f"[ContactStore] ⚠️ Legacy load error: {e}")
    ...
    def save(self) -> None:
        ...
        with _lock:
            try:
                ...
                print(f"[ContactStore] ⚠️ Encrypted save error: {e}")
