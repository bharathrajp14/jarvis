...
@register_tool(
    ...
    parameters={
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["status", "diff", "log", "branches", "create_branch", "checkout", "stage_all", "commit", "fetch", "pull", "push", "verify_remote"],
...
def git_repo_mgr(args: dict) -> str:
    ...
    if action == "status":
        ...
        return (
            f"❌ Remote verification FAILED for branch '{remote_branch}' (returncode {code}):\n{ls_out}"
        )
