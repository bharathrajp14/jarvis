...
class InputSanitizer:
    ...
    SHELL_INJECTION_PATTERNS = [
        (
            re.compile(r";\s*(rm|del|format|rd|powershell|curl|nc|bash|sh)\b", re.IGNORECASE),
            "Command chaining delimiter (;) with hazardous executable",
        ),
        (
            re.compile(r"\|\s*(curl|nc|powershell|bash|sh|wget)\b", re.IGNORECASE),
            "Pipe (|) exfiltration or remote execution",
        ),
        (
            re.compile(r"(&&|\|\|)\s*(format|rm|del|powershell|Remove-Item)\b", re.IGNORECASE),
            "Logical operator chaining with destructive action",
        ),
        ...
        (
            re.compile(r"powershell(?:\.exe)?\s+-(?:e|enc|encodedcommand)\b", re.IGNORECASE),
            "PowerShell base64 encoded command",
        ),
        (re.compile(r"cmd(?:\.exe)?\s+/[cC]\s+start\b", re.IGNORECASE), "CMD nested process launcher"),
        (
            re.compile(r"python(?:\.exe)?\s+-c\s+.*?(?:socket|subprocess|os\.system)", re.IGNORECASE),
            "Python inline socket/subprocess execution",
        ),
    ]
    ...
    PROMPT_INJECTION_DEFANG_RULES = [
        ...
        (
            re.compile(r"\b(Ignore\s+all\s+previous\s+instructions|Ignore\s+previous\s+directives)\b", re.IGNORECASE),
            "[DEFANGED_PROMPT_DIRECTIVE]",
        ),
