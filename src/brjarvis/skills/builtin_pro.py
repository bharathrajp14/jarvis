...
3. Test it with run_code:
   ...
   for test in test_cases:
       ...
       logger.info(f'{test!r} → {match.group() if match else "NO MATCH"}')
   ```
...
1. Collect system info with run_code:
   ...
   logger.info(f"CPU: {psutil.cpu_count()} cores, {psutil.cpu_percent()}% used")
...
_PRO_SKILLS = [
    ...
    (
        ...
        ["/tdd", "/test-driven"],
        ["run_code", "file_read", "file_write"],
        _TDD,
        "testing or TDD",
    ),
    (
        "code_review",
        "Thorough code review with severity levels",
        ["/code-review", "/review-code", "/cr"],
        ["file_read", "web_search", "run_code"],
        _CODE_REVIEW,
    ...
    (
        ...
        ["/refactor", "/cleanup"],
        ["file_read", "file_write", "code_refactor", "run_code"],
        _REFACTOR,
    ...
    (
        ...
        ["/dep-audit", "/audit-deps"],
        ["file_read", "run_code", "web_search", "dependency_diagnostics"],
        _DEPENDENCY_AUDIT,
    ...
    (
        ...
        ["audit_prompt_security", "audit_codebase", "run_code", "fetch_raw", "file_read"],
        _SECURITY_SCAN,
        "security scanning",
    ),
    (
        ...
        ["/osint", "/recon"],
        ["web_search", "fetch_page", "fetch_raw", "run_code"],
        _OSINT_RECON,
        "gathering OSINT",
    ),
    (
        "log_analysis",
        "Security log analysis for threats",
    ...
    (
        "ssl_check",
        "SSL/TLS certificate security audit",
        ["/ssl-check", "/ssl"],
        ["run_code", "fetch_raw", "headers_audit"],
        _SSL_CHECK,
    ...
    (
        ...
        ["/hash", "/hash-lookup"],
        ["run_code", "web_search", "file_read", "native_hash_fast"],
        _HASH_LOOKUP,
        "analyzing file hashes",
    ),
    (
        "linux_hardening",
        "Linux OS security hardening and SSH firewall audit",
        ["/hardening", "/sys-secure"],
        ["run_code", "file_read"],
        _LINUX_HARDENING,
        "hardening Linux OS",
    ),
    (
        "malware_triage",
        "Safe static binary triage and string analysis",
        ["/malware-triage", "/binary-scan"],
        ["run_code", "file_read"],
        _MALWARE_TRIAGE,
        "triage of binary samples",
    ),
    # Data
    (
        "csv_analysis",
        "Analyze CSV/JSON data files",
        ["/csv-analysis", "/data-analysis", "/analyze-data"],
        ["file_read", "run_code", "analyze_project_to_excel"],
        _CSV_ANALYSIS,
    ...
    (
        ...
        ["/chart", "/chart-gen", "/plot"],
        ["run_code", "file_read", "file_write", "create_excel_sheet"],
        _CHART_GEN,
        "generating charts",
    ),
    (
        "web_scraper_pro",
        "Extract data using HTML parsing and headless browser",
        ["/scrape", "/web-extract"],
        ["browser_auto_navigate_and_extract", "fetch_page", "file_write"],
        _WEB_SCRAPER_PRO,
        "scraping websites",
    ),
    (
        "database_migration",
        "Design database schema migration scripts",
        ["/db-migrate", "/migration"],
        ["file_write", "file_read", "run_code"],
        _DATABASE_MIGRATION,
        "database migrations",
    ),
    # DevOps
    (
        ...
        ["file_write", "file_read", "run_code"],
        _DOCKER_COMPOSE,
        "creating Docker Compose setups",
    ),
    (
        "ci_cd",
    ...
    (
        "terraform_gen",
        "Generate Terraform IaC configs",
        ["/terraform", "/tf"],
        ["file_write", "file_read"],
        _TERRAFORM_GEN,
    ...
    (
        "kubernetes_gen",
        "Generate Kubernetes manifests and deployments",
        ["/k8s", "/kubectl"],
        ["file_write", "file_read"],
        _KUBERNETES_GEN,
    ...
    (
        "performance_profile",
        "Profile CPU and memory performance bottlenecks",
        ["/profile", "/perf"],
        ["run_code", "file_read", "runtime_diagnostics"],
        _PERFORMANCE_PROFILE,
    ...
    (
        ...
        ["/async-convert", "/asyncio"],
        ["file_read", "file_write", "code_refactor"],
        _ASYNC_CONVERSION,
        "async code conversion",
    ),
    (
        "graphql_design",
    ...
    (
        "log_aggregate",
        "Aggregate and analyze distributed service logs",
        ["/log-aggregate", "/log-parse"],
        ["file_read", "run_code"],
        _LOG_AGGREGATE,
        "log aggregation",
    ),
    (
        "cloud_architecture",
        "Design cloud infrastructure solution architectures",
        ["/cloud-arch", "/cloud-design"],
        ["file_write", "document_creator"],
        _CLOUD_ARCH,
    ...
    (
        ...
        ["/doc-gen", "/docs", "/generate-docs"],
        ["document_creator", "file_read", "file_write"],
        _DOC_GEN,
    ...
    (
        "meeting_notes",
        "Structure meeting notes and action items",
        ["/meeting-notes", "/notes"],
        ["file_read", "file_write", "document_creator"],
        _MEETING_NOTES,
    ...
    (
        ...
        ["/email", "/draft-email"],
        ["send_email", "gmail_send", "file_write"],
        _EMAIL_DRAFT,
        "drafting emails",
    ),
    # System Admin
    (
        "system_info",
        "System diagnostics and health report",
        ["/system-info", "/sysinfo"],
        ["get_system_diagnostics", "system_diagnostic", "system_health", "run_code"],
        _SYSTEM_INFO,
    ...
    (
        ...
        ["/process", "/ps"],
        ["get_system_diagnostics", "kill_process", "run_code"],
        _PROCESS_MANAGER,
        "managing processes",
    ),
    (
        ...
        ["/network-diag", "/netdiag", "/ping"],
        ["run_code", "port_scan", "nmap_scan", "dns_enum"],
        _NETWORK_DIAG,
        "network diagnostics",
    ),
    (
        ...
        ["/disk-cleanup", "/disk"],
        ["system_cleanup", "get_system_diagnostics", "run_code"],
        _DISK_CLEANUP,
