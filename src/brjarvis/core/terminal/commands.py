...
class SlashCommandHandler:
    ...
    def _cmd_help(self) -> None:
        ...
        if HAS_RICH and self.renderer.console:
            ...
            # Career OS
            table.add_row("Career OS", "/career [stats|sync|onboard]", "Profile overview, completeness & funnel telemetry")
            table.add_row("Career OS", "/applications [followup]", "List tracked job applications and pending follow-ups")
            table.add_row("Career OS", "/interviews", "List scheduled interview rounds, dates, times & meeting URLs")
            table.add_row("Career OS", "/offers", "List detected job offers, compensation, terms & status")
            table.add_row("Career OS", "/emails", "View career recruitment email activity log & classifications")
    ...
    def _cmd_career(self, args_str: str = "") -> None:
        ...
        try:
            ...
            if "sync" in args_str.lower():
                ...
                self.renderer.render_markdown("⏳ _Synchronizing Canonical Database, Follow-ups, and Excel Projection..._")
                proj = get_spreadsheet_projection()
                res = proj.project_database_to_excel()
                if res.get("status") == "SUCCESS_VERIFIED":
                    self.renderer.render_markdown(f"✓ **Career Tracker Excel Projection Verified:** `{res['target_path']}` (Atomic Write Certified)")
                else:
                    self.renderer.render_markdown(f"⚠ **Excel Projection Notice:** {res}")
                return
            ...
            if "analytics" in args_str.lower() or "stats" in args_str.lower():
                self.renderer.render_markdown(f"""### 📊 BR JARVIS Career Analytics & Pipeline Telemetry:
...
* **Funnel Response Rate:** `{analytics.response_rate}%` │ **Interview Conversion:** `{analytics.interview_rate}%`
