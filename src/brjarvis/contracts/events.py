...
class AgentEvent(BaseModel):
    ...
    trace_id: str = Field(
        default_factory=lambda: f"tr-{uuid.uuid4().hex[:16]}", description="Distributed/OpenTelemetry trace ID"
    )
    execution_id: str = Field(
        default_factory=lambda: f"exec-{uuid.uuid4().hex[:12]}", description="Current execution turn ID"
    )
