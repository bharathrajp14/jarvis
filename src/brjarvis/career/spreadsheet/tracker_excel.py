...
class CareerTrackerWorkbook:
    ...
    SHEET_SCHEMAS = {
        ...
        "Contacts": ["Contact ID", "Application ID", "Company", "Name", "Title", "Email", "Phone", "LinkedIn", "Notes"],
        "Email Events": [
            "Email Event ID",
            "Application ID",
            "Message ID Hash",
        ...
        "Analytics": ["Metric / KPI", "Value", "Benchmark Target", "Status / Notes"],
        "Settings": ["Setting Key", "Setting Value", "Description", "Last Updated"],
    }
                ...
                for eml in email_events:
                    rows_data.append(
                        [
                            ...
                            eml.subject,
                            eml.received_time,
                            eml.classification.value
                            if hasattr(eml.classification, "value")
                            else str(eml.classification),
                ...
                rows_data.extend(
                    [
                        [
                            ...
                            summary.get("total_applications_submitted", len(applications)),
                            "Active Goal",
                            "Canonical DB",
                        ],
                        [
                        ...
                        [
                            ...
                            summary.get("total_interviews", len(interviews)),
                            "Active Loops",
                            "Calendar synced",
                        ],
                ...
                rows_data.extend(
                    [
                        ...
                        [
                            "followup_delay_days",
                            "6",
                            "Automatic first follow-up schedule window",
                            time.strftime("%Y-%m-%d"),
                        ],
                        [
                            "offer_auto_confirm",
                            "FALSE (STRICT_HUMAN_APPROVAL)",
                            "Safety interlock for offer state transitions",
                            time.strftime("%Y-%m-%d"),
                        ],
                        [
                            "calendar_auto_sync",
                            "TRUE",
                            "Auto-schedule verified interviews with buffer checks",
                            time.strftime("%Y-%m-%d"),
                        ],
                        [
                            "prompt_injection_defense",
                            "ACTIVE (FAIL_CLOSED)",
                            "Wraps untrusted emails & job descriptions",
                            time.strftime("%Y-%m-%d"),
                        ],
                    ]
                )
