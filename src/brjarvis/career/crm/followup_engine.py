...
from ..models import Application, FollowupRecord, PriorityLevel
...
class FollowupEngine:
    ...
    def schedule_initial_followup(
        ...
        record = FollowupRecord(
            ...
            status="PENDING",
            notes=[
                f"Scheduled automatically on {datetime.date.today().strftime('%Y-%m-%d')} for {due_date} (+{days_delay}d)"
            ],
        )

        self.db.save_followup(record)
        logger.info(
            "⏰ Scheduled follow-up #%s for [%s] %s (Due: %s)",
            record.followup_id,
    ...
    def generate_followup_draft(
        self, followup_id: str, candidate_name: str = "Bharath", recruiter_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
