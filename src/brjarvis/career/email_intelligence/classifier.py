...
class CareerEmailClassifier:
    ...
    def classify_email(
        ...
        if (
            any(kw in subj_clean for kw in ("offer letter", "job offer", "offer of employment"))
            or any(kw in body_clean for kw in offer_strong)
            or any("offer" in a for a in att_list)
        ):
        ...
        if any(
            kw in subj_clean or kw in body_clean
            for kw in ("reschedule your interview", "interview reschedule", "rescheduled:")
        ):
            return ClassificationResult(
        ...
        if any(
            kw in subj_clean
            for kw in ("interview confirmed", "interview confirmation", "calendar invite", "invitation: interview")
        ):
            return ClassificationResult(
        ...
        interview_invite_phrases = [
            ...
            "like to invite you for a technical interview",
            "technical round",
        ...
        if any(
            kw in subj_clean
            for kw in ("interview", "technical discussion", "technical round", "chat with", "next steps")
        ) or any(p in body_clean for p in interview_invite_phrases):
            return ClassificationResult(
        ...
        if any(kw in subj_clean or kw in body_clean for kw in test_phrases):
            return ClassificationResult(
                classification=EmailClassification.TECHNICAL_TEST
                if "technical" in subj_clean or "coding" in subj_clean
                else EmailClassification.ASSESSMENT,
                confidence=0.90,
        ...
        screening_phrases = [
            ...
            "time to connect for a call",
        ]
        if any(kw in subj_clean for kw in ("phone screen", "intro call", "introductory chat")) or any(
            p in body_clean for p in screening_phrases
        ):
            return ClassificationResult(
        ...
        if (
            ...
            or (
                "thank you for your interest" in body_clean
                and any(neg in body_clean for neg in ("however", "not", "unfortunately", "other candidates"))
            )
            or (
                "careful consideration" in body_clean
                and any(neg in body_clean for neg in ("however", "not", "unable", "cannot"))
            )
        ) or (
            any(
                kw in subj_clean
            ...
            and any(neg in body_clean for neg in ("not", "unfortunately", "other candidates"))
        ):
            return ClassificationResult(
        ...
        if any(
            kw in subj_clean for kw in ("application received", "thank you for applying", "application submitted")
        ) or any(p in body_clean for p in app_confirm_phrases):
            return ClassificationResult(
                classification=EmailClassification.APPLICATION_CONFIRMATION
                if "confirmation" in subj_clean or "submitted" in subj_clean
                else EmailClassification.APPLICATION_RECEIVED,
                confidence=0.94,
        ...
        recruiter_phrases = [
            ...
            "exciting role at",
            "open role at",
            "talent acquisition partner at",
            "recruiter at",
            "reach out regarding a position",
        ]
        if any(p in body_clean for p in recruiter_phrases) or any(
            kw in subj_clean for kw in ("opportunity at", "role at", "exciting position")
        ):
            return ClassificationResult(
        ...
        if any(
            kw in subj_clean or kw in body_clean
            for kw in ("withdrawn your application", "application withdrawal confirmed", "withdrew from")
        ):
            return ClassificationResult(
