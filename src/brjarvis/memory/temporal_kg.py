...
class TemporalEdge(BaseModel):
    ...
    valid_to: Optional[float] = Field(
        default=None, description="Epoch timestamp when edge was invalidated (None if currently valid)"
    )
...
class TemporalKnowledgeGraph:
    ...
    def invalidate_relation(
        self, source_id: str, relationship: str, target_id: str, invalid_at: Optional[float] = None
    ) -> bool:
    ...
    def get_entity_history(self, entity_id: str) -> List[TemporalEdge]:
        ...
        return [edge for edge in self._edges if edge.source_id == entity_id or edge.target_id == entity_id]
