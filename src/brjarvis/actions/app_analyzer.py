...
class SystemAppAnalyzer:
            ...
            for entry in s_dir.rglob("*"):
                if entry.suffix.lower() in (".lnk", ".url", ".exe"):
                    ...
                    if name.lower() not in seen_names and "uninstall" not in name.lower():
                        ...
                        results.append({"name": name, "path": str(entry), "version": "N/A", "source": "Start Menu"})
        return results
            ...
            for i in range(num_subkeys):
                try:
                    ...
                    if (
                        ...
                        or "redistributable" in name.lower()
                    ):
                        winreg.CloseKey(sub_key)
                        continue
                    ...
                    results.append(
                        {"name": str(name), "path": str(exe_path), "version": str(version), "source": "Registry"}
                    )
                    winreg.CloseKey(sub_key)
                except OSError:
                    continue
                ...
                for sub in path_obj.iterdir():
                    if sub.is_dir() and sub.name.lower() not in seen_names:
                        ...
                        if exes:
                            ...
                            results.append(
                                {"name": sub.name, "path": str(main_exe), "version": "N/A", "source": "Program Files"}
                            )
            ...
            for d_file in d_dir.glob("*.desktop"):
                try:
                    ...
                    if name and name.lower() not in seen_names:
                        seen_names.add(name.lower())
                        results.append(
                            {"name": name, "path": exec_cmd or str(d_file), "version": "N/A", "source": ".desktop file"}
                        )
                except Exception:
                    pass
        return results
        ...
        for a_dir in app_dirs:
            ...
            for app in a_dir.glob("*.app"):
                ...
                if name.lower() not in seen_names:
                    seen_names.add(name.lower())
                    results.append({"name": name, "path": str(app), "version": "N/A", "source": "Applications"})
        return results
    ...
    def search_apps(self, query: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        return {"query": query, "installed_matches": matching_installed, "running_matches": matching_running}
