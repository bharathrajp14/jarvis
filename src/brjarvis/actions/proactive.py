...
class ProactiveEngine:
    ...
    def should_trigger(self, last_user_speech: float) -> bool:
        now = time.monotonic()
        return (now - last_user_speech) >= self.min_silence_secs and (now - self._last_triggered) >= self.check_cooldown
    ...
    def build_prompt(
        ...
        if monitors:
            monitor_ctx = (
                f"\nThe user tracks these topics: {', '.join(monitors[:4])}. You may mention one if it seems relevant."
            )
