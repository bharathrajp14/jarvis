...
def _schedule_windows(target_dt: datetime, task_name: str, script_path: Path, message: str) -> str:
    ...
    if result.returncode != 0:
        ...
        logger.info(f"[Reminder] ❌ schtasks: {err}")
        return ""
...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        logger.info(f"[Reminder] ❌ launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str, script_path: Path) -> str:
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        logger.warning(f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'")
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        logger.info(f"[Reminder] ❌ at: {result.stderr.strip()}")
        return ""
...
def reminder(
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        logger.info(f"[Reminder] ❌ Scheduling exception: {e}")
