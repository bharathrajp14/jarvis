...
def test_portfolio_github_push_success_verified(tmp_path):
    ...
    gate = TaskCompletionGate(verifier=MagicMock(
    ...
    completed_steps = [
        {"step": 1, "tool": "file_write",   "status": "SUCCESS", "critical": True, "parameters": {}, "result": "portfolio created"},
        {"step": 2, "tool": "git_repo_mgr", "status": "SUCCESS", "critical": True, "parameters": {}, "result": "committed"},
        {"step": 3, "tool": "git_repo_mgr", "status": "SUCCESS", "critical": True, "parameters": {}, "result": "pushed"},
    ]
...
def test_portfolio_partial_success_when_push_fails(tmp_path):
    ...
    steps = [
        {"step": 1, "tool": "file_write",   "status": "SUCCESS", "critical": True, "parameters": {}, "result": "portfolio created"},
        {"step": 2, "tool": "git_repo_mgr", "status": "SUCCESS", "critical": True, "parameters": {}, "result": "committed"},
        {"step": 3, "tool": "git_repo_mgr", "status": "FAILED",  "critical": True, "parameters": {}, "error": "auth failure"},
    ]
...
def test_task_user_request_preserved_in_state(tmp_path):
    ...
    assert retrieved.user_request == original_goal, (
        f"user_request was modified! Original: '{original_goal[:60]}...', "
        f"Retrieved: '{retrieved.user_request[:60]}...'"
    )
...
def test_portfolio_full_real_workflow():
    ...
    for indicator in fabrication_indicators:
        assert indicator.lower() not in summary.lower(), (
            f"Response contains fabricated success language: '{indicator}'. "
            f"Full response: {summary[:300]}"
        )
    ...
    assert has_evidence, (
        f"Response does not contain any evidence markers ({evidence_markers}). "
        f"Full response: {summary[:300]}"
    )
