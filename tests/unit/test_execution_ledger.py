...
def test_ledger_append_and_retrieve(tmp_ledger):
    ...
    entry1 = LedgerEntry(
        tool_name="file_write", task_id="task_001", step_id="step_1",
        status=LedgerStatus.SUCCESS, stdout="File created at /path/portfolio.html",
        evidence="File 'portfolio.html' verified on disk (4,321 bytes).",
        verification_status=LedgerStatus.SUCCESS,
    )
    entry2 = LedgerEntry(
        tool_name="git_repo_mgr", task_id="task_001", step_id="step_2",
        status=LedgerStatus.FAILED, stderr="fatal: Authentication failed",
...
def test_ledger_step_is_verified_only_for_success(tmp_ledger):
    ...
    # Verified entry
    tmp_ledger.append(LedgerEntry(
        tool_name="file_write", task_id="t1", step_id="step_1",
        status=LedgerStatus.SUCCESS, verification_status=LedgerStatus.SUCCESS,
        evidence="File confirmed.",
    ))
    # Unverified entry
    tmp_ledger.append(LedgerEntry(
        tool_name="web_search", task_id="t1", step_id="step_2",
        status=LedgerStatus.SUCCESS, verification_status=LedgerStatus.UNVERIFIED,
    ))
    # Failed entry
    tmp_ledger.append(LedgerEntry(
        tool_name="git_repo_mgr", task_id="t1", step_id="step_3",
        status=LedgerStatus.FAILED, verification_status=LedgerStatus.FAILED,
        error="push failed",
    ))
...
def test_ledger_evidence_report_deterministic(tmp_ledger):
    ...
    tmp_ledger.append(LedgerEntry(
        tool_name="file_write", task_id="task_ev", step_id="step_1",
        status=LedgerStatus.SUCCESS, verification_status=LedgerStatus.SUCCESS,
        evidence="File portfolio.html verified (8,192 bytes).",
    ))
    tmp_ledger.append(LedgerEntry(
        tool_name="git_repo_mgr", task_id="task_ev", step_id="step_2",
        status=LedgerStatus.FAILED, verification_status=LedgerStatus.FAILED,
        evidence="Git push FAILED returncode=128",
        error="auth failure",
    ))
...
def test_ledger_task_has_critical_failure(tmp_ledger):
    ...
    tmp_ledger.append(LedgerEntry(
        tool_name="file_write", task_id="fail_task", step_id="step_1",
        status=LedgerStatus.SUCCESS, verification_status=LedgerStatus.SUCCESS,
    ))
    tmp_ledger.append(LedgerEntry(
        tool_name="git_repo_mgr", task_id="fail_task", step_id="step_2",
        status=LedgerStatus.FAILED, verification_status=LedgerStatus.FAILED,
