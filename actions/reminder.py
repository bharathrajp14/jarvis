...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    if result.returncode != 0:
        ...
        if 'logger' in globals() or 'logger' in locals():
            logger.info(f"{ f"[Reminder] ❌ schtasks: {err}" }" if isinstance(f"[Reminder] ❌ schtasks: {err}", str) else f"[Reminder] ❌ schtasks: {err}")
        else:
            import logging
            logging.getLogger(__name__).info(f"{ f"[Reminder] ❌ schtasks: {err}" }" if isinstance(f"[Reminder] ❌ schtasks: {err}", str) else f"[Reminder] ❌ schtasks: {err}")
        return ""  
...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        if 'logger' in globals() or 'logger' in locals():
            logger.info(f"{ f"[Reminder] ❌ launchctl: {result.stderr.strip()}" }" if isinstance(f"[Reminder] ❌ launchctl: {result.stderr.strip()}", str) else f"[Reminder] ❌ launchctl: {result.stderr.strip()}")
        else:
            import logging
            logging.getLogger(__name__).info(f"{ f"[Reminder] ❌ launchctl: {result.stderr.strip()}" }" if isinstance(f"[Reminder] ❌ launchctl: {result.stderr.strip()}", str) else f"[Reminder] ❌ launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        if 'logger' in globals() or 'logger' in locals():
            logger.warning(f"{ f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'" }" if isinstance(f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'", str) else f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'")
        else:
            import logging
            logging.getLogger(__name__).warning(f"{ f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'" }" if isinstance(f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'", str) else f"[Reminder] ⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'")
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        if 'logger' in globals() or 'logger' in locals():
            logger.info(f"{ f"[Reminder] ❌ at: {result.stderr.strip()}" }" if isinstance(f"[Reminder] ❌ at: {result.stderr.strip()}", str) else f"[Reminder] ❌ at: {result.stderr.strip()}")
        else:
            import logging
            logging.getLogger(__name__).info(f"{ f"[Reminder] ❌ at: {result.stderr.strip()}" }" if isinstance(f"[Reminder] ❌ at: {result.stderr.strip()}", str) else f"[Reminder] ❌ at: {result.stderr.strip()}")
        return ""

    if 'logger' in globals() or 'logger' in locals():
        logger.info("[Reminder] ❌ Neither systemd-run nor at found on this Linux system.")
    else:
        import logging
        logging.getLogger(__name__).info("[Reminder] ❌ Neither systemd-run nor at found on this Linux system.")
    return ""
...
def reminder(
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        if 'logger' in globals() or 'logger' in locals():
            logger.info(f"{ f"[Reminder] ❌ Scheduling exception: {e}" }" if isinstance(f"[Reminder] ❌ Scheduling exception: {e}", str) else f"[Reminder] ❌ Scheduling exception: {e}")
        else:
            import logging
            logging.getLogger(__name__).info(f"{ f"[Reminder] ❌ Scheduling exception: {e}" }" if isinstance(f"[Reminder] ❌ Scheduling exception: {e}", str) else f"[Reminder] ❌ Scheduling exception: {e}")
