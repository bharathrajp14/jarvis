...
class ResumeSchema:
    ...
    def to_dict(self) -> Dict[str, Any]:
        d = asdict(self)
        d["template_id"] = self.template_id.value if isinstance(self.template_id, TemplateType) else str(self.template_id)
        return d
    ...
    def from_dict(cls, data: Dict[str, Any]) -> ResumeSchema:
        ...
        if "sections" in d and isinstance(d["sections"], list):
            d["sections"] = [
                ...
                if isinstance(s, dict) else s for s in d["sections"]
            ]
